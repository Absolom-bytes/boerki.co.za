    ...
    <div className="flex-1 flex flex-col bg-surface border border-border-accent rounded shadow-sm overflow-hidden h-full">
          ...
          {messages.map(msg => {
             ...
             return (
               <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] p-3 rounded-lg text-sm ${isMe ? 'bg-accent text-white rounded-br-none' : 'bg-bg border border-border-accent text-ink rounded-bl-none'}`}>
                     {msg.content}
       ...
       <div className="p-4 bg-bg border-t border-border-accent">
          <form onSubmit={handleSend} className="flex gap-2">
             ...
             />
             <button type="submit" disabled={!content.trim()} className="bg-accent text-white px-4 rounded disabled:opacity-50">
               <Send size={18} />
             </button>
          </form>
       </div>
